...
if (!isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tblsettings WHERE id = '$id'";
...
if (isset($_POST['editAccount'])) {
    ...
    $sql = "UPDATE tblsettings SET name = '$name'  WHERE id='".$_GET['id']."'";
    ...
    date_default_timezone_set('Asia/Manila');
    $user=$_SESSION["username"];
    $action = "Updated an account record";
    $dateandtime = date("Y-m-d H:i:s");
    $sql = "INSERT INTO tblaudit (action, user, dateandtime) VALUES ('$action', '$user', '$dateandtime')";
    $conn->query($sql) or die(mysqli_error($conn));
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="post">
                    ...
                    <div class="form-group row">
                        <label class="col-md-2 col-form-label" for="username">Position:</label>
                        <div class="col-md-10">
                            <div class="form-group">
                                <input value="<?php echo "$row[position]"?>" disabled type="text" name="username" class="form-control" placeholder="Username" required>
                            </div>
                        </div>
                    </div>
